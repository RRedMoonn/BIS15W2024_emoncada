...
To import any file, first make sure that you are in the correct working directory. If you are not in the correct directory, R will not "see" the file. How do we figure out where you are?
```{r}
getwd
```
...
To allow R studio to look into folders within the folder, you do "data/"NAME OF CSV"".
```{r}
...
str(fish)
```
or you can also do this command:
```{r}
glimpse(fish)
```
Tsi gives you a glimpse of what "fish' looks like.

You can also get the names that are in "fish"
...
summary(fish)
```
Summary gives you the basic stats of "fish". tells you how big the data are and gives you the statistics like in, 1st urtile etc. This ONLY works for numeric values. 

`glimpse()` is another useful summary function.
```{r}
glimpse(fish)
```
Glimpse is clean, but the stats are not 100% trustworthy.
`nrow()` gives the numbers of rows.
```{r}
nrow(fish)
```
nrow gives total number of rows.
...
head(fish)
```
Head "gives you the first few observations of the data. 
...
tail(fish)
```
Tail gives you the bottom observations. 
...
table(fish)
```
This doesn't always work when the data is too big so you would have to specify what to look at. 
...
Gives you the same outlok as the spreadsheet ion that is found to the right of the object in the "Environment" column.
...
colnames(mammals)
```
To see what the entire data looks like:
```{r}
mammals
```
...
table(mammals$genus)
```
Notice how that said "table of extent 0"? Make sure to lok at the capitalizaiton of the column name! Genus should be capitalized since the data has it capitalized. Try again.
```{r}
table(mammals$Genus)
```
