...
The dimensions are 83 by 11 as used through the code dim(), meaning there are 83 rows and 11 columns.
...
There are 136 NAs in this data. 
...
There are 32 herbivores present in the data. 
...
```r
small_animals_bodyweight <- filter(sleep, bodywt<=19)
large_animals_bodyweight <- filter(sleep, bodywt >=200)
```
 
8. What is the mean weight for both the small and large mammals?

```r
mean(small_animals_bodyweight$bodywt, na.rm = T)
```
...
```r
mean(large_animals_bodyweight$bodywt, na.rm = T)
```
...
```r
mean(small_animals_bodyweight$sleep_total)
```
...
```r
mean(large_animals_bodyweight$sleep_total)
```
...
```
 ...
 The smaller animals sleep longer on average with a mean of 11.78. 
...
```r
sleep_times<-filter(sleep, sleep_total==19.9)
sleep_times
```
