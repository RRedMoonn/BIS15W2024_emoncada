...
"dim" stands for dimensions. "Str" stands for structure.
```{r}
dim(hbirds)
```
...
```
To make the hbirds set have his frame: 
```{r}
...
```{r}
hsprings <- data.frame(name, temp, spring)
hsprings
```
To change the name.
```{r}
data.frame(temp_C=temp, name, spring)
...
```{r}
data.frame(temp_C, scientist=name, spring)
...
Since we have an object in the environment named "temp", we can do mean(temp). this would only work if we have the object made already?
```{r}
mean(temp)
```
...
```{r}
write.csv(hbirds, "hsprings_data.csv", row.names = FALSE)
```
