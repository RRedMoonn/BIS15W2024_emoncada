...
2. Now, add a layer to this plot that fills color for each box by taxonomic class. Flling adds color codes. Filing by same cat variable you plotted by. 
```{r}
homerange %>% 
...
3. Use `group` to show the range of `log10.mass` by taxonomic class. Will group but no color is added. 
```{r}
homerange %>% 
...
homerange %>% 
  ggplot(aes(x = log10.mass)) +
  geom_histogram(color = "whitesmoke", fill = "violetred4", bins=10)+
  labs(title = "Distribution of Body Mass")
```
...
I like to see both the histogram and the density curve so I often plot them together. Note that I assign the density plot a different color. To combine them into bulk:
```{r message=FALSE}
homerange %>% 
  ggplot(aes(x=log10.mass)) +
...
```{r}
homerange %>% 
  ggplot(aes(x=log10.hra))+ #Only need to specify X. R will automatically do Y
...
homerange %>% 
  select(log10.mass)
```
Thsi is a good exmaple of a continuous variable buthwo would we make it categorical?
...
homerange %>% 
  ...
  summarise(min=min(log10.mass),
            max=max(log10.mass))
```
Another way to look at min and max is using summary.

```{r}
summary(homerange$log10.mass)
```

How would we categorize this iven how big the range is? We have some animals of larger and smaller masses. 
...
homerange %>% 
  ...
  geom_bar(position="dodge") #makes the bars side by side
```

## Practice
1. Use case_when() to make a new column `range_category` that breaks down `log10.hra` into very small, small, medium, and large classes based on quartile.Summary is a good way to get the quartiles.
```{r}
...
table(quartiles)
```
Quartiles also give you the sample size.

Now we can use this info to build the new variable.
...
homerange %>% 
  ...
  geom_bar(position="dodge", apha=0.6, color="black")
```
