...
Let's start by building a simple barplot.  Storign a simple plot as an object. Thsi allows you to layer on it to add aesthetics on the next step.
```{r}
p <- homerange %>% 
...
```{r}
p+theme_linedraw()+
  theme(legend.position = "bottom", #can change this to "top", or "bottom" or "left " or "right" . It is just an aesthetic choice
        axis.text.x = element_text(angle = 60, hjust=1))+
  labs(title = "Observations by Taxon in Homerange Data",
       x = NULL, #R is going to remove the name of the x-axis. Removes the label of the x-axis. You did not ned it because you have the name of the taxon on the bottom.  
       y= "n",
       fill= "Trophic Guild")
```
...
```{r}
p+scale_fill_brewer(palette = "PuBuGn")+ #R will still only use two of the colors in this theme since there are only two variables that are being distinguished from each other. 
  theme(legend.position = "bottom",
        axis.text.x = element_text(angle = 60, hjust=1))+
  labs(title = "Observations by Taxon in Homerange Data",
       x = NULL,
...
```{r}
my_palette <- paletteer_d("ggsci::nrc_npg") #stores your palette as an object
```

```{r}
barplot(rep(1,6), axes=FALSE, col=my_palette) #shows your palette
```

```{r}
my_palette <- paletteer_d("futurevisions::atomic_orange") #stores your palette as an object
...
```
This did not work because the color palette we chose does not have enoguh colors for out data set. It only has 3 colors so let's create a different palette.

```{r}
my_palette2 <- paletteer_d("ggsci::nrc_npg") #stores your palette as an object

```

```{r}
barplot(rep(1,6), axes=FALSE, col=my_palette2) #shows your palette
...
homerange %>% 
  ggplot(aes(x=taxon, y=log10.mass))+
  geom_boxplot()+
  facet_wrap(~trophic.guild) #Allows you to split the plot into two by carnivores and herbivores
  theme(axis.text.x = element_text(angle = 60, hjust=1))
```
...
homerange %>% 
  ggplot(aes(x=taxon, y=log10.mass))+
  geom_boxplot()+
  facet_grid(~trophic.guild~.) #sam plot but shaped differently. It is in rows not columns when you do the ~. at the end. If you begin with .~ then it does columns
  theme(axis.text.x = element_text(angle = 60, hjust=1))
```
