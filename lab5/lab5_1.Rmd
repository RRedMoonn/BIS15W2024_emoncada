...
MAKE SURE YOU ADD THE JANITOR LIBRARY TO CLEAN UP THE NAMES OF THE DATA
## Load the tidyverse
...
```{r}
## add library("janitor" above under the library("tidyverse") code.
```
...
ex. if you were interested only in variable LJ
```{r}
...
Notice that the `|` operator generates a different result.his vertical line stands for "or", so it would give you both the lakeid from AL as well as all of the length that is less than 350, regardless of the lakeid.
```{r}
filter(fish, lakeid == "AL" | length > 350)
```
...
```{r}
compared_data <- c(mammals$mass, mammals$gestation, mammals$newborn, mammals$genus=="Lophocebus","Erythrocebus","Macaca")
compared_data
```
...
He only wants to pull out the primates.
...
```{r}
mammals4 <- filter(mammals3, genus%in% c("Locphocebus", "Erythocebus", "Macaca"))
```

But this is a lot of work. Let's do soemthing more simple, which we  will learn in lab5_2.
