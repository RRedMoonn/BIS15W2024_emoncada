...
pipes shortcut is shift->command->m. 
```{r}
fish %>%
...
You can theoretically do this command and it will work but it would look messy. 

```{r}
fish %>%
  select(lakeid, scalelength) %>%  filter(lakeid == "AL")
```
...
Pipes allos you to string multiple commands!
```{r}
fish %>% #workwith the fish data
  select(lakeid, radii_length_mm) %>% #pull out variables of interest
  filter(lakeid == "AL"| lakeid=="AR") %>% #only these lakes
  filter(between(radii_length_mm, 2,4)) %>% #between two and four
  arrange(desc(radii_length_mm)) #sort to make easier to read (this is in descending order)
```
