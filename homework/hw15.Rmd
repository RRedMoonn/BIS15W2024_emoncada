...
```{r}
blood_count %>%
  pivot_wider(cols = colnames(blood_count)[3:9], names_to = "Test", values_to = "Values") %>%
...
blood_count %>%
  pivot_wider(cols = colnames(blood_count)[3:9], names_to = "Test", values_to = "Values") %>%
  blood_count %>%
  pivot_wider(names_from = c("Hemoglobin", "Platelet_Count", "White_Blood_Cells", "Red_Blood_Cells", "MCV", "MCH", "MCHC"), values_from = "Values")
  ggplot(aes(x = Gender, y = Values, fill = Gender)) +
  geom_boxplot() +
  facet_wrap(~ Test, scales = "free") +
  labs(title = "Boxplot of Blood Count Tests",
