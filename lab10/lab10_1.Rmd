...
###Look at the datascibiol example
...
homerange %>% 
  ...
  ggplot(aes(y=common.name, x=log10.mass))+ #notice the switch in x and y. you can switch thr x and y to chage the perspective of the graph. You can also do coord_flip(). Add it under this
  coord_flip()
  geom_col()
```
...
homerange %>% 
  filter(class == "mammalia") %>% 
  count(trophic.guild) %>% 
  ggplot(aes(x=trophic.guild)#y=n (do not do this))+
  geom_bar()
```

3. Make a bar plot that shows the masses of the top 10 smallest mammals.
### also look at the datascibiol example
```{r}
homerange %>% 
  filter(class=="mammalia") %>% 
  top_n(-10, log10.mass)
ggplot(aes(x-common.name, y=log10mass))
coord_flip()
```
