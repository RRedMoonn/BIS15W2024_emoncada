...
Side Note: SHINY APP DOES NOT KNIT. So for homework, you should not be able to knit.
...
ui <-fluidPage(titlePanel("log 10 Homerange by Taxon"), #title panel adds a title to the app
...
Let's go back to our original app that produced a scatterplot for the penguins data. (we just copied and pasted it from line 101)
```{r}
ui <- fluidPage(
    selectInput("x", "Select X Variable", choices = c("bill_length_mm", "bill_depth_mm", "flipper_length_mm", "body_mass_g"),
              selected = "bill_length_mm"),
...
We also pasted the server part. All we did was make the app look nicer. 
...
ui <- dashboardPage(
  ...
  dashboardBody(
  fluidRow(
  box(width = 3, #makes the box smaller. for aesthetic purposes
  ...
  dashboardBody(
    fluidRow(
      box(width = 3, #controls box width
          selectInput("x", "Select Population", choices = unique(wolves$pop)), hr() #gives some help with multiple obervations in a variable so you don't have to type all of them.
), #close the first box
...
server <- function(input, output) {
          ...
          output$plot <- renderPlot({
            wolves %>% 
              filter(sex!="NA") %>% 
              filter(pop= input$x) %>% #filter is the reactivity/reactive part
              ggplot(aes(x = sex, fill=sex)) + 
              geom_bar()
          })
...
ui <- fluidPage(    
  ...
  theme = shinytheme(),#Comment out the theme part but leave shinythemes uncommented. run the code and see which theme you like (Dropbox). Once you find the theme. Come back to the code, comment out the shinythemes line, then uncomment the theme one and write out the name of the theme as "thee = shinytheme("NAME OF THEME")
