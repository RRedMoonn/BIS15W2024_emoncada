...
Shiny function says what the ap does. Run function combines the two (ui and server) together. 
...
Select input is the dropdown menu. X is the name of the variable that will be on the x axis. sect means it will start at that input. in this case, selected is on Bill Length.
...
Side not, put spaces between the code chunks so that you dont get confused between te commas and parentheses etc. 
If you want to be able to select Y, just cope and paste the code for X and then change the x's to y's. Now we want the app to do something. This is tricky because the user interface wants to pass the input to the user side.
## Server Side
...
input$x is the interface passing to the reactive function aka the user side. Now we have to set up the plot. what plot do we want to make? we will be using ggplot and geompoint.
## Shiny Outputs
...
we want to know hat we are doing so build the plot first before adding it to the app. 
```{r, eval=F, echo=T}
ui <- fluidPage(
...
server <- function(input, output) {
  ...
  })##This bracket is closing the render plot
  
}##this brakcet is closing the server function

shinyApp(ui, server)
```
Know where your brackets are!
...
server <- function(input, output, session) {
  output$plot <- renderPlot({ #Making the Plot
