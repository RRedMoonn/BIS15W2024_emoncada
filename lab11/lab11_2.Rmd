...
Someone used "\\" as NA in the data so that is what this is. 
...
```
you can use 0.25 but that may be a bit small. Helps with teh overplotting though.  1.25 seems standard. 
We can color the points by a categorical variable.
```{r}
p + geom_point(aes(color=thermoregulation), size=(1.5)) #for scatterplots, color is the same as fill in barplots. r doesnt see a dot as an object o be filled so it needs color, not fill. if you replaced color with fill, it would not work. 
```
...
homerange %>% 
  ggplot(aes(x=taxon, fill=trophic.guild))+
  geom_bar(position="dodge")+
  theme(axis.text.x=element_text(angle=60, hjust = 1))+ #angle is the angle of the labels on the axis
  labs(title="Observations by taxon",
       x="Taxonmic Group")
```
...
homerange %>% 
  ...
  coord_flip()
```
Now you get a raw percentage of how many carnivores andherbivores are in each of these.
...
penguins %>% 
  ...
  geom_bar(position="dodge")+
  labs(title="Individuals Per Island",
       x="Island",
       fill="Species")
```
...
```{r}
penguins %>% 
  filter(sex!="NA") %>%  #removes the NAs from sex. Not really necessry
  ggplot(aes(x=island, fill=sex))+
  geom_bar(position="dodge")+
  labs(title="Individuals Divided by Sex Per Island",
       x="Island",
       y="Species")
```
...
I use `group` to make individual box plots for each taxon within class.It geos in, pull all of the taxonomic classes together to make sure that each one has it's own little box plot. 
```{r}
homerange %>% 
  ggplot(aes(x = class, y = log10.mass, group = taxon)) +
  geom_boxplot()
```
It is not helpful in this case because we dont know what these are. So you have to use fill to separate these things out to make sense a bit. 
...
homerange %>% 
  ...
  geom_boxplot(alpha=0.4) ##alpha adds transparency to see the background
```
