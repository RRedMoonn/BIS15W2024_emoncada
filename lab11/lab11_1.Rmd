...
```{r}
life_history %>% 
  ggplot(aes(x=order))+ # don't need to specify "y" because default it will be a count
  geom_bar()+
  coord_flip()
```
...
Lastly, we can adjust the x-axis labels to make them more readable. We do this using `reorder`. This is to make small on one side and big on the other.

LOOK AT CLASS COPY. something is wrong with this one
```{r}
life_history %>% 
  group_by(order) %>% 
  summarise(mean_mass=mean(mass, na.rm=T)) %>% 
...
```{r}
life_history %>% 
  filter(order!="Cetacea") %>% ##alternative is this where you pull the whales out, bu you would have to make a comment that you pulled them out
  ggplot(aes(x=order, y=mass))+
  geom_boxplot(na.rm=T)+
  coord_flip()+
  scale_y_log10()
...
elephants %>% 
  ggplot(aes(x=age, y=height)) + 
  geom_point() + 
  geom_smooth(method=lm, se=F)+ ## Adds a smoothing function. lm means linear model. se is the student error. I think it is student. 
...
elephants %>% 
  ...
  labs(title="Elephant Age vs. Height", #adds a title
       x="Age (years)",
       y="Height(cm)")+
  theme(plot.title=element_text(size=rel(1.75), hjust = 1))
```
rel=relative
hjust=horizontaljustification
rel=1.5, hjust=0.5
...
You can adjust the size of the dot by categorical data. In this case, the size of the dot is relative to the mass of the organism that is being plotted. 
