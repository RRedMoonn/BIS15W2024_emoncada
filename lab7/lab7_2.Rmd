...
glimpse(penguins)
```
To know how many islands there are, use levels(). When you run glimpse, you would see that island is a factor so using levels, you would be able to get more info.
```{r}
...
```
This function shows how many NAs are in each category. 

ow do we get rid of the NA's?
...
penguins %>% 
  ...
  summarize(mean_body_mass = mean(body_mass_g))
```
Pull out all of the observations that actually have a number and leave all of the NAs behind.
...
```
DO NOT DO THIS! DO not put the "na.rm=T" code outside of the parentheses. R will think you are trying to make a new variable.
```{r}
penguins %>% 
  group_by(species) %>% 
  summarize(mean_bill_length=mean(bill_length_mm),na.rm=TRUE)
