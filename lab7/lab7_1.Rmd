...
to see the data in the environment box you can do:

```{r}
msleep <- msleep
```

Note: Make sure to check every step before completing the entire pipe. One this works, add the "msleep24 <- msleep %>% " on the first row. Look at the second step.
```{r}
msleep %>%  
...
```{r}
msleep %>% 
  summarize(n_genera=n_distinct(genus)) # This is going to count the number of genera in the data "msleep"
```
you can check this via glmpse()

```{r}
glimpse(msleep)
```
There are 83 rows so we know that some of those genera are duplicated.
...
It is the same as the previous question. 
```{r}
msleep %>% 
...
msleep %>%
  ...
  summarize(min_bodywt = min(bodywt),
            ...
            total=n())
```
Total n is the total number of observations in each category.
